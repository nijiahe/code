...
long FAT32_read(struct file * filp,char * buf,unsigned long count,long * position)
{
	...
	do
	{	
		//color_printk(BLACK,YELLOW," BUF address = %#018x\n");
...
find_lookup_success:
	...
	char buffer[12];
	int jjj;
	for(jjj=0;jjj<11;jjj++)
	{
		buffer[jjj] = tmpdentry->DIR_Name[jjj];
	}
	buffer[11]='\0';
	

	//color_printk(WHITE,GREEN,"FILE NAME:%s, tmpdentry->DIR_FstClusHI: %#010hx, p->attribute: %#010hx",buffer,tmpdentry->DIR_FstClusHI,p->attribute);
